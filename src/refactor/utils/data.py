...
def save_fasta(df: pd.DataFrame, name_fasta: str, to_seq_groups_comparison: bool = False, number_of_sequences_to_motif_creation=1) -> str:
...
def generate_motifs_components(df: pd.DataFrame, number_of_sequences_to_motif_creation) -> dict:
    ...
    for comp, v_comp in df.groupby("TAG"):
        ...
        name_c_fasta = save_fasta(v_comp, "temp_component", to_seq_groups_comparison=True, number_of_sequences_to_motif_creation=number_of_sequences_to_motif_creation)
