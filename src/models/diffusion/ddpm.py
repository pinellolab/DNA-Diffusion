...
class DDPM(DiffusionModel):
    ...
    def p_sample_loop(self, classes, shape, cond_weight):
        ...
        if classes is not None:
            ...
            sampling_fn = partial(self.p_sample_guided, classes=classes, cond_weight=cond_weight, context_mask=context_mask)
        else:
            sampling_fn = partial(self.p_sample)
        
        for i in tqdm(reversed(range(0, self.timesteps)), desc='sampling loop time step', total=self.timesteps):
            image = sampling_fn(self.model, x=image, t=torch.full((b,), i, device=device, dtype=torch.long), t_index=i)
    ...
    def sample(self, image_size, classes=None, batch_size=16, channels=3, cond_weight=0):
        return self.p_sample_loop(self.model, classes=classes, shape=(batch_size, channels, 4, image_size), cond_weight=cond_weight)
