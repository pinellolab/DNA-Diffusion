...
def preprocess_data(
    ...
    if filter_data_bool:
        FilteringData(
            data_path, df_path, cell_list
        ).filter_exclusive_replicates(
...
def download_data(data_path: str) -> None:
    ...
    os.system(
        f"wget 'https://hgdownload.soe.ucsc.edu/goldenPath/hg38/bigZips/hg38.fa.gz' -O {tmp_dir}/hg38.fa.gz"
    )
    ...
    np.savetxt(
        f"{tmp_dir}/2018-06-08NC16_NNDSVD_Basis.csv", basis_array, delimiter=","
    )

    # Creating nmf_loadings matrix from csv
    nmf_loadings = pd.read_csv(
        f"{tmp_dir}/2018-06-08NC16_NNDSVD_Basis.csv", header=None
    )
    ...
    np.savetxt(
        f"{tmp_dir}/2018-06-08NC16_NNDSVD_Mixture.csv",
        mixture_array,
        delimiter=",",
    )
    ...
    os.system(
        f"gunzip -d {tmp_dir}/DHS_Index_and_Vocabulary_hg38_WM20190703.txt.gz"
    )
...
def create_master_dataset(
    ...
    DHS_Index_and_Vocabulary_metadata = pd.read_table(
        f"{tmp_dir}/DHS_Index_and_Vocabulary_metadata.tsv"
    ).iloc[:-1]
    ...
    # Creating nmf_loadings matrix from csv
    basis_nmf_loadings = pd.read_csv(
        f"{tmp_dir}/2018-06-08NC16_NNDSVD_Basis.csv", header=None
    )
    ...
    DHS_Index_and_Vocabulary_metadata = pd.concat(
        [DHS_Index_and_Vocabulary_metadata, basis_nmf_loadings], axis=1
    )
    ...
    sequence_metadata = pd.read_table(
        f"{tmp_dir}/DHS_Index_and_Vocabulary_hg38_WM20190703.txt", sep="\t"
    )
    ...
    mixture_nmf_loadings = pd.read_csv(
        f"{tmp_dir}/2018-06-08NC16_NNDSVD_Mixture.csv",
        header=None,
        names=component_columns,
    )
    # Join metadata with component presence matrix
    df = pd.concat(
        [sequence_metadata, mixture_nmf_loadings], axis=1, sort=False
    )
    ...
    # Opening file
    binary_matrix = pd.read_table(
        f"{tmp_dir}/dat_bin_FDR01_hg38.txt", header=None
    )
        ...
        for k, v in df_subset.groupby("TAG"):
            if k != "NO_TAG":
                ...
                v["additional_replicates_with_peak"] = (
                    self.df[self.df.filter(like=cell).columns].apply(
