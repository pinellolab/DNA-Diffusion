...
def sampling_to_metric(model, cell_types, image_size, nucleotides, number_of_samples=20, specific_group=False, 
                        group_number=None, cond_weight_to_metric=0):
    """ 
    ...
    for n_a in tqdm(range(number_of_samples)):
        ...
        sampled_images = model.sample(classes=random_classes, image_size=image_size, batch_size=sample_bs, channels=1, cond_weight=cond_weight_to_metric)
...
def metric_comparison_between_components(original_data, generated_data, x_label_plot, y_label_plot, cell_components):
    """
