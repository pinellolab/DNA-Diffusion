...
def save_dataset(origin_dataset, save_dir):
    ...
    for key in tqdm(origin_dataset[0].keys(), total=len(origin_dataset[0].keys())):
...
def generate_dataset(positive_file, negative_file, genome_file, sample_length=1000):
    ...
    with open(positive_file, 'r') as f_pos:
        print('Generating positive trainable dataset...')
        for line in tqdm(f_pos, total=get_file_length(positive_file)):
            chrom = line.rstrip('\n').split('\t')[0]
    ...
    f_pos.close()
    print('Positive dataset generation is finished!')
    print('Generating negative trainable dataset...')
    with open(negative_file, 'r') as f_neg:
        for line in tqdm(f_neg, total=get_file_length(negative_file)):
            chrom = line.rstrip('\n').split('\t')[0]
            start = int(line.rstrip('\n').split('\t')[1])
            end = int(line.rstrip('\n').split('\t')[2])
            mid = (start + end) / 2
    ...
    f_neg.close()
    print('Negative dataset generation is finished!')
    return dataset
